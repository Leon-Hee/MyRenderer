...
void render::RenderTriangles(Triangles& t, Framebuffer& framebuffer, Depthbuffer& depthbuffer, const Mat4& MVP, int width, int height, const Mat4& M, const Mat4& V, const Light& light, const Vector3f& cameraPos, const Material& material, const ShadowMap& shadowMap, const Mat4& lightMVP){
    ...
    for(auto& i : clipList){
        ...
        if(isFront(vert0, vert1, vert2)){
            rasterizer.drawTriangles(clippedTri, framebuffer, depthbuffer, light, cameraPos, material, shadowMap, lightMVP);
        }
    }
